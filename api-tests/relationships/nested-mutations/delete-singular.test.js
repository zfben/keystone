...
multiAdapterRunners().map(({ runner, adapterName }) =>
  describe(`Adapter: ${adapterName}`, () => {
    describe('no access control', () => {
      test(
        'removes matched item from list & database',
        ...
        mutation {
          updateEvent(
            id: "${createEvent.id}"
            data: {
              group: { delete: { id: "${createGroup.id}" } }
            }
          ) {
            id
            group {
    ...
    `,
          ...
          // Check the item was also deleted from the database
          const groupData = await findById('Group', createGroup.id);
          expect(groupData).toBe(null);
        })
      );
    ...
    `,
      ...
      test(
        'silently succeeds if no item to delete during update',
    ...
    `,
      ...
      test(
        'silently succeeds if item to delete does not match during update',
        runner(setupKeystone, async ({ keystone, create }) => {
          const groupName = `foo${sampleOne(alphanumGenerator)}`;
          const FAKE_ID = '5b84f38256d3c2df59a0d9bf';
    ...
    describe('with access control', () => {
      describe('read: false on related list', () => {
        test(
          'has no effect when delete a specific id',
          runner(setupKeystone, async ({ keystone, create, findById }) => {
